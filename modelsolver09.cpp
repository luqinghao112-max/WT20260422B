 ...
 * 功能描述:
 * 1. 混积型（三重孔隙）非均质裂缝计算核心（多表皮独立分配终极版）。
 * 2. 精确执行基于跟端绝对距离 (xw) 向几何中心映射的积分坐标域。
 * 3. 使用 phi_ct 合并缩放与高精度系数 16.118095 执行有因次转换。
 * 4. PWD_composite 全面使用各段独立 S_vec 进行 2nf+1 维系统的等效流场阻力施加。
 */
...
double ModelSolver09::safe_bessel_k_sc(int v, double x) {
    if (x < 1e-15) x = 1e-15;
    if (x > 600.0) return std::sqrt(M_PI / (2.0 * x));
    try { return boost::math::cyl_bessel_k(v, x) * std::exp(x); } catch (...) { return 0.0; }
}

double ModelSolver09::safe_bessel_i_sc(int v, double x) {
    if (x < 0) x = -x;
...
void ModelSolver09::setHighPrecision(bool h) { m_highPrecision = h; }

QString ModelSolver09::getModelName(ModelType t, bool v) {
    int id = (int)t + 1;
    return QString("独立表皮非均质混积型模型%1").arg(id);
}
...
// 页岩 Tanh 扩散核
double ModelSolver09::calc_fs_shale(double u, double o, double l) {
...
// ================= 5. 【终极版核心】包含独立表皮向量的 2nf+1 维封闭大矩阵 =================
double ModelSolver09::PWD_composite(double z, double fs1, double fs2, double M12,
    ...
    // 突破点：局部流动压降使用 S_eff_vec[i] 赋予独立约束
    for (int i = 0; i < n_fracs; ++i) {
        int row = n_fracs + i;
