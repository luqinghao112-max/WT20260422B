 ...
 * 功能描述:
 * 1. 模型 5 (对应MATLAB V8.2 Number5) 页岩型非均质裂缝核心计算实现。
 * 2. 具备 2nf+1 维干扰矩阵计算机制，支持每条缝不同的长度与位置干涉叠加。
 * 3. 强制组合 phi_ct 用于无因次时间转换，引入高精度 16.118095 系数进行真实压力转换。
 * 4. 内置稳健的压敏非线性泰勒外推，保障对数导数曲线平滑。
 */
...
double ModelSolver05::safe_bessel_k_scaled(int v, double x) {
    if (x < 1e-15) x = 1e-15;
    if (x > 600.0) return std::sqrt(M_PI / (2.0 * x)); // 大自变量渐近展开
...
double ModelSolver05::safe_bessel_i_scaled(int v, double x) {
    if (x < 0) x = -x;
    if (x > 600.0) return 1.0 / std::sqrt(2.0 * M_PI * x); // 大自变量渐近展开
    try { return boost::math::cyl_bessel_i(v, x) * std::exp(-x); } catch (...) { return 0.0; }
}

// 构造函数：初始化并预计算 8 阶 Stehfest 系数
...
QString ModelSolver05::getModelName(ModelType type, bool verbose) {
    int id = (int)type + 1;
    QString baseName = QString("不等长非均匀页岩型试井模型%1").arg(id);
    return baseName;
}
...
double ModelSolver05::flaplace_composite(double z, const QMap<QString, double>& p) {
    ...
    if (id <= 12) {
        ...
        double omga2 = p.value("omega2", 0.08);
        double remda2 = p.value("lambda2", 1e-4);
        fs2 = eta12 * calc_fs_dual(z_outer, omga2, remda2); // 双重孔隙
    }
...
// 【核心求解器】：构建含有非等长裂缝的 2nf+1 维干扰矩阵并求解
double ModelSolver05::PWD_composite(double z, double fs1, double fs2, double M12,
    ...
    // ========== 1. 组装空间流场干扰积分矩阵 (基于 Gauss-Kronrod) ==========
    ...
    // 第二部分矩阵：连接局部缝面压降与全局统一井底流压（施加表皮压降）
    for (int i = 0; i < n_fracs; ++i) {
        int row = n_fracs + i;
        A_sys(row, i) = -S_eff;
    ...
    // 自适应 Tikhonov 正则化处理，防止空间网格密集导致矩阵奇性引发数值崩溃
